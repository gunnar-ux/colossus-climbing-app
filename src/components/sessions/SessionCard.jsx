    ...
    <div 
      className={`${
        isExceptionalSession 
          ? 'bg-card border border-cyan-900/40 relative overflow-hidden' 
          : 'bg-card border border-border'
      } rounded-col px-4 pt-4 pb-3 cursor-pointer`} 
      onClick={() => setOpen(!open)}
    >
      {/* Ice blue gradient background for exceptional sessions */}
      {isExceptionalSession && (
        <div className="absolute inset-0 bg-gradient-to-br from-cyan-950/30 via-transparent to-transparent pointer-events-none" />
      )}
      
      <div className="relative">
        {/* Header Row: Session Focus with optional trophy + Climbs count */}
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            <div className="font-semibold text-base text-white">{sessionFocus}</div>
            {isExceptionalSession && (
        ...
        <div className={`flex items-center justify-center pt-2 ${isExceptionalSession ? 'border-t border-cyan-900/40' : 'border-t border-border/40'}`}>
          <span className={`text-xs ${isExceptionalSession ? 'text-cyan-400/50' : 'text-graytxt/60'}`}>powered by </span>
          <span className={`text-xs font-semibold ml-1 ${isExceptionalSession ? 'text-white' : 'text-white'}`}>POGO</span>
          <ChevronDownIcon 
            className={`w-4 h-4 transition-transform duration-200 ml-auto ${isExceptionalSession ? 'text-cyan-400/50' : 'text-graytxt/60'} ${open ? 'rotate-180' : ''}`}
          />
        </div>
      </div>
